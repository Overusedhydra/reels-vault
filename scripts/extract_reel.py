...
Each extracted reel is tagged with topic + creator so it auto-organizes in your
Obsidian vault. Run `scripts/connect.py` once to point this at your vault.

Usage:
    python3 scripts/extract_reel.py <reel_url> [--topic content-creation]

Dependencies (auto-installed by setup.sh):
    brew install ffmpeg yt-dlp        # or apt-get on Linux
...
def extract_metadata(url: str, output_dir: str, cookies_from: str = None, session_file: str = None) -> tuple:
    ...
    # Locate the info json (filename may differ too)
...
def identify_music(audio_path: str) -> dict:
    """Identify the music track via Shazam. Returns {} if nothing found."""
...
def format_output(metadata: dict, transcript: dict, original_url: str = "",
    ...
    # Frontmatter — what makes reels filterable/groupable in Obsidian
    fm = ["---"]
    ...
    lines = list(fm)
    lines += ["", "# Instagram Reel Extraction", ""]
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract transcript and metadata from an Instagram Reel into your vault."
    )
    ...
    parser.add_argument("--topic", default=None,
                        help="Topic to file this reel under (e.g. 'content-creation'). "
                             "Creates a <topic>/ folder so reels group by topic.")
    ...
    try:
        ...
        if args.json:
            output = json.dumps(
                {"metadata": metadata, "transcript": transcript, "music": music},
        ...
        # Where does the reel land?
        #   1. --save-dir wins
        #   2. else the connected vault (see scripts/connect.py)
        #   3. else just print to stdout
